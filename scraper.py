# thank you to vlmaier for his scraper. this is heavily inspired by
# https://github.com/vlmaier/marvel-snap-scrapr/blob/main/scrapr.py
...
def download_images(urls, dir_name='marvel-snap-cards'):
    ...
    for url in urls:
        time.sleep(1)
        download_image(url=url, dir_name=dir_name)
...
def create_cards(cards):
    ...
    for card in cards:
        ...
        card_dict[parse_name(card["name"])] = body
    return card_dict
...
if __name__ == '__main__':
    ...
    # download_images(image_urls, dir_name="./images")
    characters = scrap()
    card_dict = create_cards(characters)
    today = date.today()
    format_date = today.strftime("%m_%d_%y")
    with open(f"./characters/characters_{format_date}.json", "w") as outfile: 
        json.dump(card_dict, outfile, indent=4)